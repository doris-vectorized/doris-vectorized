...
template <typename T>
ColumnPtr ColumnVector<T>::filter_by_selector(const uint16_t* sel, size_t sel_size, ColumnPtr* ptr) {
    if (ptr == nullptr) {
        auto res = this->create();
        if (sel_size == 0) {
            return res;
        ...
        Container& res_data = res->get_data();
        //todo(wb) vectorized here
        for (size_t i = 0; i < sel_size; i++) {
            res_data.push_back(data[sel[i]]);
        }
        return res;
    } else {
        if (sel_size == 0) {
        ...
        //todo(wb) add DCHECK type
        auto& cv_ref = reinterpret_cast<vectorized::ColumnVector<T>&>(*ptr_res);
        Container& res_data = cv_ref.get_data();
        for (size_t i = 0; i < sel_size; i++) {
            res_data.push_back(data[sel[i]]);
        }
        return nullptr;
...
template <typename T>
ColumnPtr ColumnVector<T>::filter_date_by_selector(const uint16_t* sel, size_t sel_size, ColumnPtr* ptr) {
    if (ptr == nullptr) {
        auto res = this->create();
        if (sel_size == 0) {
            return res;
