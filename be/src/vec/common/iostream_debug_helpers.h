...
std::enable_if_t<priority == 2, Out>& dumpImpl(
        Out& out, T&& x, std::decay_t<decltype(std::begin(std::declval<T>()))>* = nullptr) {
    bool first = true;
    out << "{";
...
template <int priority, typename Out, typename T>
std::enable_if_t<priority == 3 && (std::is_same_v<std::decay_t<T>, std::string> ||
...
template <int priority, typename Out, typename T>
std::enable_if_t<priority == 3 && std::is_same_v<std::decay_t<T>, unsigned char>, Out>& dumpImpl(
        Out& out, T&& x) {
    return out << int(x);
}
...
std::enable_if_t<priority == 4, Out>& dumpImpl(
        Out& out, T&& x, std::decay_t<decltype(std::get<0>(std::declval<T>()))>* = nullptr) {
...
#define DUMP7(V1, V2, V3, V4, V5, V6, V7)                                                \
    do {                                                                                 \
        DUMPHEAD DUMPVAR(V1) DUMPVAR(V2) DUMPVAR(V3) DUMPVAR(V4) DUMPVAR(V5) DUMPVAR(V6) \
                DUMPVAR(V7) DUMPTAIL                                                     \
    } while (0)
#define DUMP8(V1, V2, V3, V4, V5, V6, V7, V8)                                            \
    do {                                                                                 \
        DUMPHEAD DUMPVAR(V1) DUMPVAR(V2) DUMPVAR(V3) DUMPVAR(V4) DUMPVAR(V5) DUMPVAR(V6) \
                DUMPVAR(V7) DUMPVAR(V8) DUMPTAIL                                         \
    } while (0)
#define DUMP9(V1, V2, V3, V4, V5, V6, V7, V8, V9)                                        \
    do {                                                                                 \
        DUMPHEAD DUMPVAR(V1) DUMPVAR(V2) DUMPVAR(V3) DUMPVAR(V4) DUMPVAR(V5) DUMPVAR(V6) \
                DUMPVAR(V7) DUMPVAR(V8) DUMPVAR(V9) DUMPTAIL                             \
    } while (0)
