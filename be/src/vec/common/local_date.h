...
public:
    ...
    operator time_t() const { return DateLUT::instance().makeDate(m_year, m_month, m_day); }

    DayNum getDayNum() const { return DateLUT::instance().makeDayNum(m_year, m_month, m_day); }
    ...
    std::string toString(char separator = '-') const {
        std::stringstream ss;
        if (separator)
            ss << year() << separator << (month() / 10) << (month() % 10) << separator
               << (day() / 10) << (day() % 10);
        else
...
inline std::ostream& operator<<(std::ostream& ostr, const LocalDate& date) {
    return ostr << date.year() << '-' << (date.month() / 10) << (date.month() % 10) << '-'
