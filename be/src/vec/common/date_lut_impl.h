...
#define DATE_LUT_YEARS \
    (1 + DATE_LUT_MAX_YEAR - DATE_LUT_MIN_YEAR) /// Number of years in lookup table
...
private:
    ...
    inline DayNum findIndex(time_t t) const {
        ...
        if ((guess == 0 || t >= lut[guess].date) && t < lut[DayNum(guess + 1)].date) return guess;
        ...
        if (offset_at_start_of_epoch >= 0) return DayNum(guess + 1);
...
public:
    ...
    inline time_t toTime(time_t t) const {
        ...
        if (UNLIKELY(index == 0)) return t + offset_at_start_of_epoch;
    ...
    inline unsigned toMinute(time_t t) const {
        if (offset_is_whole_number_of_hours_everytime) return (t / 60) % 60;
    ...
    inline time_t toStartOfHour(time_t t) const {
        if (offset_is_whole_number_of_hours_everytime) return t / 3600 * 3600;
    ...
    */
    inline YearWeek toYearWeek(DayNum d, UInt8 week_mode) const {
        ...
        if (toMonth(d) == 1 && toDayOfMonth(d) <= static_cast<UInt32>(7 - weekday)) {
            if (!week_year_mode &&
                ((first_weekday_mode && weekday != 0) || (!first_weekday_mode && weekday >= 4)))
                return yw;
            week_year_mode = 1;
            (yw.first)--;
    ...
    inline YearWeek toYearWeekOfNewyearMode(DayNum d, bool monday_first_mode) const {
        ...
        if (monday_first_mode) {
            ...
            if (toDayOfWeek(first_day) != 7) first_day = DayNum(first_day - toDayOfWeek(first_day));
            if (toDayOfWeek(d) != 7) this_day = DayNum(d - toDayOfWeek(d));
        }
    ...
    inline time_t toRelativeHourNum(time_t t) const {
        if (offset_is_whole_number_of_hours_everytime) return t / 3600;
    ...
    inline DayNum toStartOfYearInterval(DayNum d, UInt64 years) const {
        if (years == 1) return toFirstDayNumOfYear(d);
    ...
    inline time_t toStartOfHourInterval(time_t t, UInt64 hours) const {
        ...
        t = t / seconds * seconds;
        if (offset_is_whole_number_of_hours_everytime) return t;
        return toStartOfHour(t);
    }

    inline time_t toStartOfMinuteInterval(time_t t, UInt64 minutes) const {
        if (minutes == 1) return toStartOfMinute(t);
    ...
    inline DayNum makeDayNum(UInt16 year, UInt8 month, UInt8 day_of_month) const {
        if (UNLIKELY(year < DATE_LUT_MIN_YEAR || year > DATE_LUT_MAX_YEAR || month < 1 ||
                     month > 12 || day_of_month < 1 || day_of_month > 31))
            return DayNum(0);

        return DayNum(years_months_lut[(year - DATE_LUT_MIN_YEAR) * 12 + month - 1] + day_of_month -
                      1);
    }
    ...
    /** Does not accept daylight saving time as argument: in case of ambiguity, it choose greater timestamp.
      */
    inline time_t makeDateTime(UInt16 year, UInt8 month, UInt8 day_of_month, UInt8 hour,
                               UInt8 minute, UInt8 second) const {
    ...
    inline UInt64 toNumYYYYMMDDhhmmss(time_t t) const {
        const Values& values = find(t);
        return toSecond(t) + toMinute(t) * 100 + toHour(t) * 10000 +
               UInt64(values.day_of_month) * 1000000 + UInt64(values.month) * 100000000 +
               UInt64(values.year) * 10000000000;
    }

    inline time_t YYYYMMDDhhmmssToTime(UInt64 num) const {
        return makeDateTime(num / 10000000000, num / 100000000 % 100, num / 1000000 % 100,
                            num / 10000 % 100, num / 100 % 100, num % 100);
    }
    ...
    inline UInt8 saturateDayOfMonth(UInt16 year, UInt8 month, UInt8 day_of_month) const {
        if (LIKELY(day_of_month <= 28)) return day_of_month;

        UInt8 days_in_month = daysInMonth(year, month);

        if (day_of_month > days_in_month) day_of_month = days_in_month;
