...
struct HashTableProbeIntersect {
    HashTableProbeIntersect(VIntersectNode* intersect_node, int batch_size, int probe_rows)
            : _intersect_node(intersect_node),
              _left_table_data_types(intersect_node->_left_table_data_types),
              _batch_size(batch_size),
              _probe_rows(probe_rows),
              _probe_block(intersect_node->_probe_block),
              _probe_index(intersect_node->_probe_index),
              _num_rows_returned(intersect_node->_num_rows_returned),
              _probe_raw_ptrs(intersect_node->_probe_columns),
              _arena(intersect_node->_arena),
              _rows_returned_counter(intersect_node->_rows_returned_counter),
              _build_col_idx(intersect_node->_build_col_idx) {}

    Status mark_data_in_hashtable(HashTableContext& hash_table_ctx) {
        using KeyGetter = typename HashTableContext::State;
        using Mapped = typename HashTableContext::Mapped;

        KeyGetter key_getter(_probe_raw_ptrs, _intersect_node->_probe_key_sz, nullptr);
        ...
        for (; _probe_index < _probe_rows;) {
            ...
            if (find_result.is_found()) { //if found, marked visited
                ...
                if (!(it->visited)) {
                    it->visited = true;
                    _intersect_node->_valid_element_in_hash_tbl++;
                }
            }
            _probe_index++;
        }
    ...
    Status get_data_in_hashtable(HashTableContext& hash_table_ctx,
        ...
        for (; iter != hash_table_ctx.hash_table.end() && block_size < _batch_size; ++iter) {
            ...
            if (it->visited) { //have done probe, so visited values it's the result
                block_size++;
...
private:
    VIntersectNode* _intersect_node;
