...
struct SubtractSecondsImpl : SubtractIntervalImpl<AddSecondsImpl> { static constexpr auto name = "seconds_sub"; };
struct SubtractMinutesImpl : SubtractIntervalImpl<AddMinutesImpl> { static constexpr auto name = "minutes_sub"; };
struct SubtractHoursImpl : SubtractIntervalImpl<AddHoursImpl> { static constexpr auto name = "hours_sub"; };
struct SubtractDaysImpl : SubtractIntervalImpl<AddDaysImpl> { static constexpr auto name = "days_sub"; };
struct SubtractWeeksImpl : SubtractIntervalImpl<AddWeeksImpl> { static constexpr auto name = "weeks_sub"; };
struct SubtractMonthsImpl : SubtractIntervalImpl<AddMonthsImpl> { static constexpr auto name = "months_sub"; };
struct SubtractQuartersImpl : SubtractIntervalImpl<AddQuartersImpl> { static constexpr auto name = "quarters_sub"; };
struct SubtractYearsImpl : SubtractIntervalImpl<AddYearsImpl> { static constexpr auto name = "years_sub"; };
...
struct DateTimeOp
{
    ...
    static void vector_vector(const PaddedPODArray<FromType> & vec_from0, const PaddedPODArray<FromType> & vec_from1,
    ...
    static void vector_constant(const PaddedPODArray<FromType> & vec_from, PaddedPODArray<ToType> & vec_to, Int128& delta)
    {
    ...
    static void vector_constant(const PaddedPODArray<FromType> & vec_from, PaddedPODArray<ToType> & vec_to, Int64 delta)
    {
        size_t size = vec_from.size();
        vec_to.resize(size);
    ...
    static void constant_vector(const FromType & from, PaddedPODArray<ToType> & vec_to, const IColumn & delta)
    {
        size_t size = delta.size();
        vec_to.resize(size);

        for (size_t i = 0; i < size; ++i)
            vec_to[i] = Transform::execute(from, delta.getInt(i));
    }

    // use for (const DateTime, DateTime) -> other_type
    static void constant_vector(const FromType& from, PaddedPODArray<ToType>& vec_to, const PaddedPODArray<Int128>& delta)
    {
        size_t size = delta.size();
        vec_to.resize(size);
...
//        const DateLUTImpl & time_zone = extractTimeZoneFromFunctionArguments(block, arguments, 2, 0);
        ...
        if (const auto * sources = checkAndGetColumn<ColumnVector<FromType>>(source_col.get()))
        {
            ...
            if (const auto *delta_const_column = typeid_cast<const ColumnConst *>(&delta_column)) {
                ...
                const auto *delta_vec_column = checkAndGetColumn<ColumnVector<FromType>>(delta_column);
                Op::vector_vector(sources->getData(), delta_vec_column->getData(), col_to->getData());
            }
        ...
        {
            auto col_to = ColumnVector<ToType>::create();
            if (const auto *delta_vec_column = checkAndGetColumn<ColumnVector<FromType>>(*block.getByPosition(arguments[1]).column)) {
                Op::constant_vector(sources_const->template getValue<FromType>(), col_to->getData(), delta_vec_column->getData());
            } else {
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        if (arguments.size() != 2 && arguments.size() != 3)
            throw Exception("Number of arguments for function " + getName() + " doesn't match: passed "
                + std::to_string(arguments.size()) + ", should be 2 or 3",
