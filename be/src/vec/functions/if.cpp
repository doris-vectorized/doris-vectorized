...
public:
    ...
    bool executeForNullThenElse(Block & block,
        ...
        if (then_is_null && else_is_null) {
            block.get_by_position(result).column = block.get_by_position(result).type->createColumnConstWithDefaultValue(input_rows_count);
            return true;
        }
    ...
    bool executeForNullableThenElse(Block & block, 
...

        {
            Block temporary_block(
            {
                ...
                {
                    nullptr,
                    removeNullable(block.get_by_position(result).type),
                    ""
                }
            });
    ...
    Status executeImpl(Block & block, const ColumnNumbers & arguments, size_t result, size_t input_rows_count) override {
        ...
        if (cond_const_col) {
            block.get_by_position(result).column = cond_const_col->get_value<UInt8>()
                ? arg_then.column
                : arg_else.column;
            return Status::OK();
        }
