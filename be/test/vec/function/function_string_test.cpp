...
// test vec reverse
TEST(function_string_reverse_test,function_string_reverse_test) {
        int len = 7;
        std::string str_test[len] = {"", "a", "美团和和阿斯顿百度ab", "!^%", "ò&ø", "A攀c", "NULL"};
        std::string str_expected[len] = {"", "a", "ba度百顿斯阿和和团美", "%^!", "ø&ò", "c攀A", "LLUN"};
        
        auto str_col = vectorized::ColumnString::create();
        for (int i = 0; i < len; ++i) {
            str_col->insert_data(str_test[i].c_str(), str_test[i].size());
        }
        
        vectorized::DataTypePtr data_type(std::make_shared<vectorized::DataTypeString>());
        vectorized::ColumnWithTypeAndName type_and_name(str_col->get_ptr(), data_type, "k1");
        vectorized::Block block({type_and_name});
        ...
        auto str_function = doris::vectorized::SimpleFunctionFactory::instance().get_function(
                "reverse", ctn, data_type);
        size_t num_columns_without_result = block.columns();
        block.insert({nullptr, block.get_by_position(0).type, "reverse(k1)"});
